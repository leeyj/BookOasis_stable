...
* 유연한 스캔 예외 필터링 (.bookoasisignore)
  - 시놀로지 NAS 썸네일 폴더(`@eaDir/`), 휴지통(`#recycle/`), 임시 파일(`*.tmp`, `*.sample.cbz`) 등 예외 패턴을 전역 설정 또는 폴더별 `.bookoasisignore` 파일로 등록하여 불필요한 스캔 및 디스크 탐색 소모를 물리적으로 차단합니다.
...
## 💾 Redis 캐시 연동 및 성능 가이드 (선택 및 권장)

BookOasis는 실시간 독서 진행도 저장 시 SQLite 파일에 바로 접근하지 않고 인메모리(RAM) 캐시로 완충해 주는 **Redis 하이브리드 캐시 모드**를 지원합니다.
이를 통해 디스크 쓰기 병목이 격감하고 스캔 중의 DB 손상(Malformed) 현상이 완벽히 방지됩니다.
...
1. 호스트 OS에 레디스 서버를 설치 및 기동합니다. (Ubuntu 예시: `sudo apt install redis-server -y`)
2. `.env` 파일 하단에 접속 URL을 기재해 줍니다.
   ```env
   REDIS_URL=redis://127.0.0.1:6379/9
   ```
   * **Tip**: 다른 기존 서비스와 레디스를 공유하는 경우, DB 번호를 `/9`번 등 다른 인덱스로 지정하면 충돌이 전혀 없습니다. 모든 데이터 키 앞에 `bookoasis:` 네임스페이스가 강제 지정됩니다.
3. 레디스가 준비되지 않았거나 접속에 실패하면 **자동으로 기존의 SQLite 직접 쓰기 모드로 우회(Fallback)** 하므로 안심하고 사용하셔도 됩니다.
