---
title: Task - session_20260722_192622
project: BookOasis
category: history
date: 2026-07-22
type: task
...
- [x] 아이패드 및 태블릿 브라우저 하단 여백 및 잘림 현상 개선 (style.css, mobile.css, library_dashboard.html)
- [x] 스캐너 워커 Redis brpop 유휴 고착 및 DB 큐 미감지 버그 조치 (redis_helper.py, scanner_queue.py)
- [x] 1회 처리용량 제한 달성 후 연쇄 스캔 자동 재개 버그 조치 (lazy_scanner.py, scanner_queue.py)
- [x] 메타데이터 파싱과 DB 트랜잭션의 완전 분리를 통한 DB Malformed 원천 방지 (lazy_scanner.py)
- [ ] 일반 환경설정 탭 성격별 4대 섹션(UI/스캐너/시스템/보안) 구조화 및 UI 리팩토링 (general_tab.html)
