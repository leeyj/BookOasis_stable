...
### ②-1 오디오북 시간 정보(0:00) 정확도 보강 권장사항
오디오북 스캐너는 기본 파서(`mutagen`, `tinytag`)로 길이를 계산하고, 계산값이 `0.0`인 경우 `ffprobe`로 파일을 직접 열어 2차 확인합니다.

* **네이티브 설치 사용자(Windows/Linux/macOS)**:
    * `ffmpeg`(내부 포함 `ffprobe`)를 시스템에 설치하는 것을 권장합니다.
    * `ffmpeg`가 없으면 서비스는 동작하지만, 일부 WMA/특수 포맷에서 `0:00` 복구율이 낮아질 수 있습니다.

* **도커 사용자**:
    * 프로젝트의 최신 `Dockerfile`에는 `ffmpeg`가 포함되어 있습니다.
    * 소스 빌드 경로는 이미지 재빌드 후 반영됩니다: `docker compose up --build`
    * GHCR 이미지 경로는 최신 태그를 `pull` 후 재기동하십시오.
...
server {
    ...
    # 5. 커버 이미지 라우팅 최적화
    # 5-1. 동적 SVG 커버 생성 API (백엔드 파이썬으로 전달)
    ...
    # 5-2. 커버 이미지 정적 서빙 최적화 (파이썬 우회 다이렉트 0ms 서빙)
    location /covers/ {
