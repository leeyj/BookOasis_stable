...
* **동작 방식**:
  - 특정 폴더 진입 시 해당 폴더의 수정 시간(`dir_mtime = os.path.getmtime(root)`)과 폴더 내부에 잔존하는 메타데이터 파일(`kavita.yaml`, `info.xml` 등)들의 수정 시간 중 최댓값(`meta_mtime = max(meta_mtimes_list)`)을 개별 추출합니다.
  - 이 두 가지 값(`dir_mtime`, `meta_mtime`)을 DB `folder_mtimes` 테이블의 캐시 레코드와 동시에 비교 대조합니다.
  - 두 조건이 모두 일치하고 하위 파일들이 온전히 캐싱된 상태라면 **가장 무거운 도서 압축파일 파서(Parser) 로직 자체를 호출하지 않고 해당 폴더 전체를 0.001초 만에 즉시 스킵(Return)**하여 스캔 병목을 완벽하게 제거합니다.
