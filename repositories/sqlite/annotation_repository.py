# -*- coding: utf-8 -*-
"""
annotation_repository.py – EPUB/TXT 뷰어 하이라이트(주석) CRUD 데이터 액세스 레이어.
"""
import database
...
class AnnotationRepository:
    ...
    def get_annotation_by_id(db_type, annotation_id, user_id=None):
        ...
        try:
            if user_id is not None:
                cursor.execute("SELECT * FROM book_annotations WHERE id = ? AND user_id = ?", (annotation_id, user_id))
            else:
                cursor.execute("SELECT * FROM book_annotations WHERE id = ?", (annotation_id,))
            row = cursor.fetchone()
    ...
    def update_annotation(db_type, annotation_id, user_id, color, note):
        ...
        try:
            cursor.execute(
                ...
                SET color = ?, note = ?, updated_at = CURRENT_TIMESTAMP
    ...
    def update_plugin_marker(db_type, annotation_id, user_id, marker):
        ...
        try:
            cursor.execute(
                "UPDATE book_annotations SET plugin_marker = ? WHERE id = ? AND user_id = ?",
    ...
    def delete_annotation(db_type, annotation_id, user_id):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("DELETE FROM book_annotations WHERE id = ? AND user_id = ?", (annotation_id, user_id))
            conn.commit()
            return cursor.rowcount > 0
        except Exception as e:
            conn.rollback()
