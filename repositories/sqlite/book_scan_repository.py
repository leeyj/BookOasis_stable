...
class BookScanRepository:
    ...
    def fill_empty_book_metadata(db_type, book_id, fields):
        ...
        try:
            ...
            cursor.execute(
                f"SELECT COALESCE(metadata_locked, 0) AS is_locked, {', '.join(columns)} FROM books WHERE id = ?",
                (book_id,)
            )
            row = cursor.fetchone()
            ...
            assignments = ', '.join(
                f"{column} = CASE WHEN {empty_guard_sql(column)} THEN ? ELSE {column} END"
                for column in to_fill
            )
            cursor.execute(
                f"UPDATE books SET {assignments} WHERE id = ? AND COALESCE(metadata_locked, 0) = 0",
