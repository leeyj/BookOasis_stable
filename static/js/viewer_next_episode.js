  ...
  fetch(url)
    ...
    .then(data => {
      if (data.success && data.next_book) {
        ...
        if (action === 'direct') {
          console.log(`[Viewer-Next] Option: direct. Opening next book id=${nextBook.id} immediately.`);
          triggerOpenNextBook(nextBook);
        } else {
          console.log(`[Viewer-Next] Option: prompt. Displaying next episode confirmation modal.`);
